...
Symbol* insertSymbol(int type, string text, bool isTemp)
{
    ...
    Symbol* s = new Symbol(type, text, isTemp);
    symbolsTable[text] = s;
    return s;
}
...
Symbol* makeTemp()
{
    static int serial = -1;
    ++serial;
    return insertSymbol(SYMBOL_VARIABLE, "TEMP_SYMBOL" + std::to_string(serial), true);
}
